      ...
      <div className="select-cont">
        {shipsToDeploy.map((ship) => {
          ...
          if (ship.gameField.rows > 1) {
            field.addShip(ship, [2, ship.gameField.rows > 3 ? 0 : 1]);
          } else field.addShip(ship, [ship.gameField.cols > 3 ? 0 : 1, 2]);
          return (
            <div
              onContextMenu={(e) => {
                e.preventDefault();
                setShipsToDeploy((prev) => {
                  return prev.map((x) => {
                    if (x.id === ship.id) {
                      return {
                        ...x,
                        gameField: x.gameField.rotateShip(),
                      };
                    }
                    return x;
                  });
              ...
              className={` select-item  ${
                selectedId === ship.id ? "select-cont-selected" : ""
              }`}
              key={ship.id}
            >
              <div>
